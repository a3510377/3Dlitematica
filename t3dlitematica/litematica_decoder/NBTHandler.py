...
def ReadCompoundTag(
    binSource: bytes,
    indentationWhiteSpace: int = 2,
    pointer: int = 0,
    defaultIndentation: int = 0,
    readName: bool = False,
    ...
    while True:
        ...
        if binSource[pointer] == 1:  # TAG_Byte
            jsonLine, pointer = ReadByteTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonLine["name"]] = jsonLine["value"]
        elif binSource[pointer] == 2:  # TAG_Short
            jsonLine, pointer = ReadShortTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonLine["name"]] = jsonLine["value"]
        elif binSource[pointer] == 3:  # TAG_Int
            jsonLine, pointer = ReadIntTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonLine["name"]] = jsonLine["value"]
        elif binSource[pointer] == 4:  # TAG_Long
            jsonLine, pointer = ReadLongTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonLine["name"]] = jsonLine["value"]
        elif binSource[pointer] == 5:  # TAG_Float
            jsonLine, pointer = ReadFloatTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonLine["name"]] = jsonLine["value"]
        elif binSource[pointer] == 6:  # TAG_Double
            jsonLine, pointer = ReadDoubleTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonLine["name"]] = jsonLine["value"]
        elif binSource[pointer] == 7:  # TAG_Byte_Array
            jsonSubLines, pointer = ReadByteArrayTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonSubLines["name"]] = jsonSubLines["value"]
        elif binSource[pointer] == 8:  # TAG_String
            jsonLine, pointer = ReadStringTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonLine["name"]] = jsonLine["value"]
        elif binSource[pointer] == 9:  # TAG_List
            jsonSubLines, pointer = ReadListTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonSubLines["name"]] = jsonSubLines["value"]
        elif binSource[pointer] == 10:  # TAG_Compound
            jsonSubLines, pointer = ReadCompoundTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2, True
            )
            litematicdata[jsonSubLines["name"]] = jsonSubLines["value"]
        elif binSource[pointer] == 11:  # TAG_Int_Array
            jsonSubLines, pointer = ReadIntArrayTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonSubLines["name"]] = jsonSubLines["value"]
        elif binSource[pointer] == 12:  # TAG_Long_Array
            jsonSubLines, pointer = ReadLongArrayTag(
                copy.deepcopy(binSource), indentationWhiteSpace, pointer, indentation + 2
            )
            litematicdata[jsonSubLines["name"]] = jsonSubLines["value"]
        else:
...
def ReadListTag(
    ...
    if contentType != 0:
        if contentType == 1:  # TAG_Bytes
            ...
            for _ in range(contentCount):
                jsonSubLines, pointer = ReadListTag(
                    binSource, indentationWhiteSpace, pointer, defaultIndentation + 2, True
                )
                jsonLines["value"].append(jsonSubLines)
        elif contentType == 10:  # TAG_Compound
            for _ in range(contentCount):
                jsonSubLines, pointer = ReadCompoundTag(
                    binSource, indentationWhiteSpace, pointer - 3, defaultIndentation + 2, False
                )
                jsonLines["value"].append(jsonSubLines)
        elif (
            contentType == 7 or 10 < binSource[pointer] < 13
